...
class sparameter_base ( wx.Dialog ):

	def __init__( self, parent ):
		wx.Dialog.__init__ ( self, parent, id = wx.ID_ANY, title = u"s-parameter", pos = wx.DefaultPosition, size = wx.Size( 913,640 ), style = wx.DEFAULT_DIALOG_STYLE|wx.MAXIMIZE_BOX|wx.RESIZE_BORDER )
		...
		sbSizer3.Add( self.m_listBoxFpAdded, 1, wx.EXPAND|wx.FIXED_MINSIZE, 5 )
...
class s_parameter_gui(sparameter_base):
    def __init__(self):
        sparameter_base.__init__(self, None)
        
        self.cfg_list = []
        self.cur_cfg = sp_config_item()
        ...
        self.output_path = self.board_path + os.sep + "s_parameter" 
        self.cfg_path = self.board_path + os.sep + "s_parameter" + os.sep + "s_parameter.json"
    ...
    def load_cfg(self):
        try:
            ...
            self.cfg_list.append(sp_config_item())
    ...
    def gen_cmd(self, all = True):
        ...
        for cfg in cfg_list:
            cmd = cmd + "z_extractor -pcb " + self.board.GetFileName() + " -sp "
    ...
    def m_buttonGenerateOnButtonClick( self, event ):
        ...
        self.thread = threading.Thread(target=s_parameter_gui.thread_func, args=(self,))
        self.thread.start()
    ...
    def m_buttonRunOnButtonClick( self, event ):
        ...
        if sys == "Windows":
            ...
            cmd_line = term + '"cd ' + self.output_path + ' && octave --silent --persist ' + self.cur_cfg.name + '_sp.m '
...
class s_parameter( pcbnew.ActionPlugin ):
    def defaults( self ):
